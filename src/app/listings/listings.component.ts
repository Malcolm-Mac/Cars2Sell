...
export class ListingsComponent implements OnInit {
  ...
  schema = {
    ...
    "@type": "Product",
    "name": "this.vehicleData[0].MAKE",
    "image": 'this.vehicleData[0].vehicleImage',
    "offers": {
      "@type": "AggregateOffer",
      "url": "",
      "priceCurrency": "ZAR",
      "lowPrice": "this.vehicleData[0].PRICE"
    }
  };
  ...
  ngOnInit(): void {
    ...
    this.vehicleData = this.vehicleDataService.getAllVehicle().sort(() => Math.random() - 0.5)
    ...
    if (this.vehicleData.length <= 0) {
      ...
      this.setAllDataForComponentToStart()
    }
  }

  setAllDataForComponentToStart() {
  ...
  viewSort() {
    this.controlDropDown("sortBYDiv")
  }
  sortByFunction(sortBY: string, data: string) {
    this.tempSearch = this.searchInput;
    this.searchInput = ""
    if (this.dealername != "" && this.dealername != null) {
      this.vehicleData = this.vehicleDataService.sortBy(sortBY).filter(item => item.DEALERNAME.toLowerCase() == this.dealername.toLocaleLowerCase())
    }
    else {
  ...
  revelNumber(type: string, contactDetails: string) {
    if (type == "Phone") {
      if (!this.returnHrefNumber) {
        let phoneNumber = this.dealerData.TELEPHONENUMBER.length ? this.dealerData.TELEPHONENUMBER : this.dealerData.PHONENUMBER
        this.showwNumberDeatails = this.isNumber(phoneNumber)
